  ...
  .object({
    filters: z
      .object({
        ...
        balance_type: z.array(z.string()).optional(),
        seller_status: z.array(z.string()).optional(),
  ...
  for (const key of groupBy) {
    switch (key) {
      ...
      case 'warehouse':
        ...
        break;
      case 'balance_type':
        dims.push({ baseExpr: "COALESCE(t.balance_type, '__UNKNOWN__')", alias: 'balance_type' });
        break;
      case 'sku':
        dims.push({ baseExpr: "COALESCE(t.product_sku, '__UNKNOWN__')", alias: 'product_sku' });
        break;
      case 'barcode':
        dims.push({ baseExpr: "COALESCE(t.barcode, '__UNKNOWN__')", alias: 'barcode' });
        break;
      case 'snapshot_date':
        dims.push({ baseExpr: 'CAST(t.snapshot_date AS VARCHAR)', alias: 'snapshot_date' });
        break;
    }
  }
...
export function registerShopifyListInventoryBalancesTool(registry: ToolRegistry) {
  ...
  registry.register({
    ...
    description:
      'List Shopify inventory balance records with warehouse and seller details. Supports filtering, grouping, and aggregation.',
    ...
    execute: async (args, context) => {
      ...
      if (query.sort?.field) {
        warnings.push('Server-side sorting is not implemented yet; using default sort (snapshot_date DESC, stock_quantity DESC).');
      }

      // ---- Determine aggregation mode ----
      const validGroupByKeys = new Set(['company', 'seller', 'warehouse', 'balance_type', 'sku', 'barcode', 'snapshot_date']);
      ...
      const skuList = Array.isArray(filters.sku)
        ...
        : [];
      const barcodeList = Array.isArray(filters.barcode)
        ? filters.barcode.map((s: any) => String(s).trim()).filter((s: string) => s.length > 0)
        : [];
      const warehouseList = Array.isArray(filters.warehouse_name)
        ? filters.warehouse_name.map((s: any) => String(s).trim()).filter((s: string) => s.length > 0)
        : [];
      const sellerList = Array.isArray(filters.seller_name)
        ? filters.seller_name.map((s: any) => String(s).trim()).filter((s: string) => s.length > 0)
        : [];
      const balanceTypeList = Array.isArray(filters.balance_type)
        ? filters.balance_type.map((s: any) => String(s).trim()).filter((s: string) => s.length > 0)
        : [];
      const sellerStatusList = Array.isArray(filters.seller_status)
        ? filters.seller_status.map((s: any) => String(s).trim()).filter((s: string) => s.length > 0)
        : [];
      const inventoryIdList = Array.isArray(filters.inventory_id)
        ? filters.inventory_id.map((n: any) => Number(n)).filter((n: number) => Number.isFinite(n) && n > 0)
        : [];
      ...
      const commonTemplateVars: Record<string, string | number> = {
        ...
        barcodes_array: sqlVarcharArrayExpr(barcodeList),
        warehouse_names_array: sqlVarcharArrayExpr(warehouseList),
        warehouse_names_lower_array: sqlVarcharArrayExpr(warehouseList.map((s: string) => s.toLowerCase())),
        seller_names_array: sqlVarcharArrayExpr(sellerList),
        seller_names_lower_array: sqlVarcharArrayExpr(sellerList.map((s: string) => s.toLowerCase())),
        balance_types_array: sqlVarcharArrayExpr(balanceTypeList),
        balance_types_lower_array: sqlVarcharArrayExpr(balanceTypeList.map((s: string) => s.toLowerCase())),
        seller_statuses_array: sqlVarcharArrayExpr(sellerStatusList),
        seller_statuses_lower_array: sqlVarcharArrayExpr(sellerStatusList.map((s: string) => s.toLowerCase())),
        inventory_ids_array: sqlBigintArrayExpr(inventoryIdList),

        date_from_sql: filters.date_from ? `DATE '${sqlEscapeString(String(filters.date_from))}'` : 'CAST(NULL AS DATE)',
        date_to_sql: filters.date_to ? `DATE '${sqlEscapeString(String(filters.date_to))}'` : 'CAST(NULL AS DATE)',
        snapshot_date_sql: filters.snapshot_date
          ? `DATE '${sqlEscapeString(String(filters.snapshot_date))}'`
          : 'CAST(NULL AS DATE)',
      };
      ...
      if (isAggregated) {
        ...
        // Enforce company_id filter client-side
