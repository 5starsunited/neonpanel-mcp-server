...
sqp_raw AS (
  ...
  WHERE q.company_id = CAST(p.company_id AS VARCHAR)
    AND LOWER(q.marketplace_country_code) = LOWER(p.marketplace)
...
screenshots AS (
  ...
  CROSS JOIN params p
  LEFT JOIN UNNEST(s.competitors) AS t(comp_row) ON TRUE
  WHERE s.company_id = p.company_id
