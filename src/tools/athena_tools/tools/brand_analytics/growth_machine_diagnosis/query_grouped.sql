-- brand_analytics_growth_machine_diagnosis - GROUPED (ClickHouse)
-- Generated from query.sql: everything up to focus_filtered is identical, then
-- the detail projection is replaced by intent enrichment + group-level rollup.
-- Keep the two in sync; the CTE prefix is intentionally byte-identical.
...
intent_enriched AS (
    SELECT
        f.*,
        ifNull(ti.primary_intent_id, '__UNCLASSIFIED__') AS intent_id,
        ti.primary_intent_label AS intent_label
    FROM focus_filtered AS f
...
aggregated AS (
    SELECT
        ...
        count() AS row_count,
        uniqExact(e.kw_norm) AS keyword_count,
        uniqExact(e.child_asin) AS asin_count,
        -- Prescription distribution
        countIf(e.prescription = 'FIX_CART_LEAK_CUT_PPC') AS n_fix_cart_leak,
        countIf(e.prescription = 'NEGATIVE_EXACT') AS n_negative_exact,
        countIf(e.prescription = 'INJECT_INTO_SEO') AS n_inject_into_seo,
        countIf(e.prescription = 'DEFEND_ORGANIC') AS n_defend_organic,
        countIf(e.prescription = 'DISPLACE_WEAK_LEADER') AS n_displace_weak_leader,
        countIf(e.prescription = 'EVALUATE_OR_SKIP') AS n_evaluate_or_skip,
        -- Signal counts
        countIf(e.sig_cart_leak) AS n_sig_cart_leak,
        countIf(e.sig_bleeder) AS n_sig_bleeder,
        countIf(e.sig_proven_winner) AS n_sig_proven_winner,
        countIf(e.sig_cannibalization) AS n_sig_cannibalization,
        countIf(e.sig_weak_leader) AS n_sig_weak_leader,
        countIf(e.sig_defend) AS n_sig_defend,
        -- Averaged KPIs (mean over detail rows in each bucket). avg() skips
        -- NULLs, so a bucket with no PPC attribution yields NULL, not 0.
        ...
        -- Summed spend / volume
        sum(ifNull(e.ppc_spend, 0)) AS total_ppc_spend,
        sum(ifNull(e.ppc_sales, 0)) AS total_ppc_sales,
        sum(ifNull(e.ppc_clicks, 0)) AS total_ppc_clicks
    FROM intent_enriched AS e
    GROUP BY {{group_by_clause}}
)

SELECT *
FROM aggregated
-- row_count is an explicit tiebreaker: with PPC attribution unavailable every
-- total_ppc_spend is 0, and without it the "top" buckets would be arbitrary.
ORDER BY total_ppc_spend DESC, row_count DESC
LIMIT {{limit_top_n}}
