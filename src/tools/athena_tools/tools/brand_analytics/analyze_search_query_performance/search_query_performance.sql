...
SELECT
     fb.*,
     -- WoW / WoLast4 / WoLast12 window deltas
     fb.kpi_impression_share - LAG(fb.kpi_impression_share) OVER (
          PARTITION BY fb.company_id, fb.marketplace_country_code, fb.searchquerydata_searchquery, fb.row_type, fb.parent_asin, fb.asin
          ORDER BY fb.week_start
     ) AS kpi_impression_share_wow,
     fb.kpi_impression_share - AVG(fb.kpi_impression_share) OVER (
          PARTITION BY fb.company_id, fb.marketplace_country_code, fb.searchquerydata_searchquery, fb.row_type, fb.parent_asin, fb.asin
          ORDER BY fb.week_start
     ...
     fb.kpi_click_share - LAG(fb.kpi_click_share) OVER (
          PARTITION BY fb.company_id, fb.marketplace_country_code, fb.searchquerydata_searchquery, fb.row_type, fb.parent_asin, fb.asin
          ORDER BY fb.week_start
     ) AS kpi_click_share_wow,
     fb.kpi_click_share - AVG(fb.kpi_click_share) OVER (
          PARTITION BY fb.company_id, fb.marketplace_country_code, fb.searchquerydata_searchquery, fb.row_type, fb.parent_asin, fb.asin
          ORDER BY fb.week_start
          ROWS BETWEEN 4 PRECEDING AND 1 PRECEDING
     ...
     fb.kpi_ctr_advantage - LAG(fb.kpi_ctr_advantage) OVER (
          PARTITION BY fb.company_id, fb.marketplace_country_code, fb.searchquerydata_searchquery, fb.row_type, fb.parent_asin, fb.asin
          ORDER BY fb.week_start
     ) AS kpi_ctr_advantage_wow,
     fb.kpi_ctr_advantage - AVG(fb.kpi_ctr_advantage) OVER (
          PARTITION BY fb.company_id, fb.marketplace_country_code, fb.searchquerydata_searchquery, fb.row_type, fb.parent_asin, fb.asin
          ORDER BY fb.week_start
          ROWS BETWEEN 4 PRECEDING AND 1 PRECEDING
