-- Athena smoke test for inventory_valuation_analyze_shipment_discrepancy.
-- Edit the params CTE values before running in Athena.
-- This checks the same shipped-vs-received discrepancy logic used by the MCP tool.

WITH params AS (
  SELECT
    CAST(ARRAY[82] AS ARRAY(BIGINT)) AS company_ids,
    DATE '2026-01-01' AS start_date,
    DATE '2026-05-07' AS end_date,
    'shipped_at' AS date_field,
    false AS discrepancy_only,
    0 AS quantity_tolerance,
    false AS include_inactive,
    false AS include_cancelled_deleted
),
...
fba_inbound_ledger AS (
  ...
  FROM awsdatacatalog.neonpanel_iceberg.amazon_fba_inbound_shipment_ledger fba
...
shipment_lines AS (
  ...
  FROM awsdatacatalog.neonpanel_iceberg.inventory_shipment_items isi
  LEFT JOIN awsdatacatalog.neonpanel_iceberg.inventory_shipments ishp ON ishp.id = isi.shipment_id
  LEFT JOIN awsdatacatalog.neonpanel_iceberg.amazon_fba_inbound_shipments afis ON afis.id = ishp.document_id
  LEFT JOIN awsdatacatalog.neonpanel_iceberg.inventory_items iio ON iio.id = isi.original_inventory_id
  LEFT JOIN awsdatacatalog.neonpanel_iceberg.app_products apo ON apo.id = iio.product_id
  LEFT JOIN awsdatacatalog.neonpanel_iceberg.amazon_marketplaces amo ON amo.id = iio.marketplace_id
  LEFT JOIN awsdatacatalog.neonpanel_iceberg.inventory_warehouses iw1 ON iw1.id = ishp.original_warehouse_id
  LEFT JOIN awsdatacatalog.neonpanel_iceberg.inventory_warehouses iw2 ON iw2.id = ishp.destination_warehouse_id
  LEFT JOIN awsdatacatalog.neonpanel_iceberg.app_companies ac ON ac.id = ishp.company_id
  ...
  LEFT JOIN awsdatacatalog.neonpanel_iceberg.inventory_shipments ishp ON ishp.id = isi.shipment_id
  LEFT JOIN awsdatacatalog.neonpanel_iceberg.amazon_fba_inbound_shipments afis ON afis.id = ishp.document_id
  LEFT JOIN awsdatacatalog.neonpanel_iceberg.inventory_items iid ON iid.id = isi.destination_inventory_id
  LEFT JOIN awsdatacatalog.neonpanel_iceberg.app_products apd ON apd.id = iid.product_id
  LEFT JOIN awsdatacatalog.neonpanel_iceberg.amazon_marketplaces amd ON amd.id = iid.marketplace_id
  LEFT JOIN awsdatacatalog.neonpanel_iceberg.inventory_warehouses iw1 ON iw1.id = ishp.original_warehouse_id
  LEFT JOIN awsdatacatalog.neonpanel_iceberg.inventory_warehouses iw2 ON iw2.id = ishp.destination_warehouse_id
  LEFT JOIN awsdatacatalog.neonpanel_iceberg.app_companies ac ON ac.id = ishp.company_id
...
ORDER BY absolute_discrepancy DESC, shipped_at DESC, shipment_id DESC
LIMIT 100;
