...
-- Purpose: Aggregate shipment item shipped-vs-received discrepancies.
...
aggregated AS (
  SELECT
    ...
    SUM(f.quantity_shipped) AS quantity_shipped,
    SUM(f.quantity_received) AS quantity_received,
    SUM(f.quantity_discrepancy) AS quantity_discrepancy,
    ABS(SUM(f.quantity_discrepancy)) AS absolute_discrepancy,
    CASE
      WHEN SUM(f.quantity_shipped) = 0 THEN NULL
      ELSE ROUND((CAST(SUM(f.quantity_received) AS DOUBLE) / CAST(SUM(f.quantity_shipped) AS DOUBLE)) * 100, 2)
    END AS received_rate_percent,
    COUNT(*) AS shipment_line_count,
    COUNT(DISTINCT f.shipment_id) AS shipment_count,
    COUNT(DISTINCT f.seller_sku) AS sku_count,
    COUNT(DISTINCT CASE WHEN f.absolute_discrepancy > p.quantity_tolerance THEN f.shipment_item_id ELSE NULL END) AS discrepant_line_count,
    MIN(CAST(f.selected_date AS VARCHAR)) AS earliest_selected_date,
    MAX(CAST(f.selected_date AS VARCHAR)) AS latest_selected_date
  FROM filtered f
  CROSS JOIN params p
...
SELECT
  ...
  {{select_dimensions}}
  ag.quantity_shipped,
  ag.quantity_received,
  ag.quantity_discrepancy,
  ag.absolute_discrepancy,
  ag.received_rate_percent,
  ag.shipment_line_count,
  ag.shipment_count,
  ag.sku_count,
  ag.discrepant_line_count,
  ag.earliest_selected_date,
  ag.latest_selected_date
FROM aggregated ag
{{order_by_clause}}
LIMIT {{limit_rows}}
