  ...
  .object({
    filters: z
      .object({
        ...
        doc_numbers: z
          .array(z.string())
          .min(1, 'At least one doc_number is required')
          .max(20, 'Maximum 20 doc_numbers per request'),
      })
      .strict(),
  })
  .strict();
...
export function registerFinancialsGetJournalEntryDetailsTool(registry: ToolRegistry) {
  ...
  registry.register({
    ...
    description:
      'Gets the full line-by-line breakdown of specific journal entries identified by doc_number. Shows every account line with debit/credit, account info, and attachment filenames.',
