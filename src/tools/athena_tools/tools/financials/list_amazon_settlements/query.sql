...
WHERE
  ...
  -- deposit_date is a string like "16.02.2026 07:12:11 UTC" → parse then convert to LA
  AND (p.start_date IS NULL OR CAST(AT_TIMEZONE(DATE_PARSE(s.deposit_date, '%d.%m.%Y %H:%i:%s UTC'), 'America/Los_Angeles') AS DATE) >= p.start_date)
  AND (p.end_date   IS NULL OR CAST(AT_TIMEZONE(DATE_PARSE(s.deposit_date, '%d.%m.%Y %H:%i:%s UTC'), 'America/Los_Angeles') AS DATE) <= p.end_date)
