  ...
  .object({
    filters: z
      .object({
        ...
        report_charts: z.array(z.string()).optional(),
    ...
    time: z
      ...
      .optional(),
    sort_direction: z.enum(['asc', 'desc']).default('asc').optional(),
...
export function registerFinancialsListAccountTransactionsTool(registry: ToolRegistry) {
  ...
  registry.register({
    ...
    description:
      'Lists individual journal entry lines for specific accounts – the detailed account ledger. Shows JE name, ref number (doc_number), date, debit, credit, description, and running balance per account.',
    ...
    execute: async (args, context) => {
      ...
      const reportCharts = (query.filters.report_charts ?? []).map((s) => s.trim()).filter(Boolean);
      ...
      const descriptions = (query.filters.descriptions ?? []).map((s) => s.trim()).filter(Boolean);
      ...
      const rendered = renderSqlTemplate(template, {
        ...
        report_charts_array: sqlVarcharArrayExpr(reportCharts),
        ...
        descriptions_array: sqlVarcharArrayExpr(descriptions),
