...
SELECT
  order_id,
  DATE_FORMAT(created_time    AT TIME ZONE 'UTC', '%Y-%m-%dT%H:%i:%sZ') AS created_time,
  DATE_FORMAT(last_updated_time AT TIME ZONE 'UTC', '%Y-%m-%dT%H:%i:%sZ') AS last_updated_time,
  marketplace_id,
  marketplace_name,
  fulfillment_status,
  fulfilled_by,
  service_level,
  DATE_FORMAT(ship_by_earliest AT TIME ZONE 'UTC', '%Y-%m-%dT%H:%i:%sZ') AS ship_by_earliest,
  DATE_FORMAT(ship_by_latest   AT TIME ZONE 'UTC', '%Y-%m-%dT%H:%i:%sZ') AS ship_by_latest,
  order_total,
  currency,
  item_count,
  company_id,
