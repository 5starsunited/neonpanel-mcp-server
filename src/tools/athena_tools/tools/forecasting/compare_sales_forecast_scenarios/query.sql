...
forecast_latest_rows AS (
  SELECT
    ...
    i.marketplace_key AS marketplace_key, i.child_asin AS child_asin, i.parent_asin AS parent_asin,
    i.asin AS asin, i.product_name AS product_name, i.product_family AS product_family,
    i.brand AS brand, i.snapshot_date AS snapshot_date,
...
forecast_rows AS (
  SELECT
    rows.company_id, rows.inventory_id, rows.sku, rows.marketplace_key, rows.child_asin,
    rows.parent_asin, rows.asin, rows.product_name, rows.product_family, rows.brand,
    rows.snapshot_date, rows.series_type, rows.scenario_name, rows.run_updated_at, rows.period,
    toInt64(round(sum(coalesce(toFloat64(rows.units_sold), 0.0)), 0)) AS units_sold,
    round(sum(coalesce(toFloat64(rows.sales_amount), 0.0)), 2) AS sales_amount,
    if(sum(coalesce(toFloat64(rows.units_sold), 0.0)) > 0,
      round(sum(coalesce(toFloat64(rows.sales_amount), 0.0)) / sum(coalesce(toFloat64(rows.units_sold), 0.0)), 3),
      CAST(NULL, 'Nullable(Float64)')) AS unit_price,
    min(rows.currency) AS currency
  FROM forecast_latest_rows AS rows
  GROUP BY rows.company_id, rows.inventory_id, rows.sku, rows.marketplace_key, rows.child_asin,
    rows.parent_asin, rows.asin, rows.product_name, rows.product_family, rows.brand,
    rows.snapshot_date, rows.series_type, rows.scenario_name, rows.run_updated_at, rows.period
),
...
actual_rows AS (
  SELECT
    ...
    if(sum(coalesce(toFloat64(rows.units_sold), 0.0)) > 0,
      ...
      CAST(NULL, 'Nullable(Float64)')) AS unit_price,
    min(rows.currency) AS currency
  FROM actual_latest_rows AS rows
  GROUP BY rows.company_id, rows.inventory_id, rows.sku, rows.marketplace_key, rows.child_asin,
    rows.parent_asin, rows.asin, rows.product_name, rows.product_family, rows.brand,
    rows.snapshot_date, rows.series_type, rows.scenario_name, rows.run_updated_at, rows.period
),
...
SELECT
  rows.company_id, rows.inventory_id, rows.sku, rows.marketplace_key AS marketplace,
  rows.child_asin, rows.parent_asin, rows.asin, rows.product_name, rows.product_family,
  rows.brand, rows.snapshot_date, rows.series_type, rows.scenario_name,
  rows.run_updated_at, rows.period, rows.units_sold, rows.sales_amount, rows.unit_price, rows.currency
FROM base_rows AS rows
ORDER BY rows.period ASC, rows.scenario_name ASC, rows.series_type ASC, rows.run_updated_at DESC
LIMIT {{limit_top_n}}
