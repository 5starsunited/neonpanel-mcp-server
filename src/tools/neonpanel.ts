...
export function registerNeonPanelTools(registry: ToolRegistry) {
  registry
    ...
    .register({
      name: 'account_get_companies_with_permission',
      description:
    ...
    .register({
      name: 'inventory_get_listing_details_by_asin',
      description:
      ...
      execute: async (args, context) => {
        ...
        // Backend: ListingController::paginateByCompany — reads asin/search
        // AND pagination params from the query string. The 2026-02-19 OpenAPI
        // doc describes a JSON body but the live controller still paginates,
        // so we pass everything as query params.
    ...
    .register({
      name: 'import_get_instructions',
      description: 'Retrieve document upload instructions for a supported import type (NeonPanel: GET /api/v1/import/{type}/instructions).\n\nReturns: attributes (object with parameter names as keys and descriptions as values) and general (text with specifications and tips).\n\nCurrently only type="bill" is supported. Use the returned attribute names/descriptions to build the JSON payload for import_create_documents.\n\nWorkflow: 1) Call this tool → 2) Build document data object → 3) Call import_create_documents or import_create_documents_by_pdf.',
      isConsequential: false,
    ...
    .register({
      name: 'import_create_documents',
      description: 'Create documents for a company from a structured JSON payload (NeonPanel: POST /api/v1/companies/{uuid}/create/{type}).\n\nReturns: request_id (UUID), status ("queued"/"processing"/"done"/"error"), and documents[] array of created documents.\n\nThe data object structure must match the schema from import_get_instructions. Currently only type="bill" is supported.\n\nWorkflow: 1) import_get_instructions → 2) Build data payload → 3) This tool → 4) If status is not "done", poll with import_check_status using the returned request_id.',
      isConsequential: true,
