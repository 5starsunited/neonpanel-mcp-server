...
const NEONPANEL_API_SCHEME_URL = 'https://my.neonpanel.com/api/v1/scheme';
const CACHE_DURATION = 5 * 60 * 1000; // 5 minutes cache
let schemaCache = null;
/**
 * Fetches the latest NeonPanel API OpenAPI schema
 */
async function fetchNeonPanelSchema() {
    try {
        console.log('🔄 Fetching fresh NeonPanel API schema...');
        const response = await axios_1.default.get(NEONPANEL_API_SCHEME_URL, {
            timeout: 10000,
            headers: {
        ...
        console.log('✅ NeonPanel API schema fetched successfully');
        return response.data;
    }
    catch (error) {
        console.error('❌ Failed to fetch NeonPanel API schema:', error);
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
        throw new Error(`Failed to fetch API schema: ${errorMessage}`);
    }
}
/**
...
function getFallbackCapabilities() {
    ...
    return [
        {
            capability_name: 'neonpanel_inventory',
            description: 'NeonPanel inventory management capabilities (fallback)',
            actions: [
                {
                    action_id: 'get_items',
