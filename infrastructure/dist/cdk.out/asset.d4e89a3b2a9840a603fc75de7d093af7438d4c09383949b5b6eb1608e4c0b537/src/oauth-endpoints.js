...
function buildAuthorizationServerMetadata(baseUrl) {
    const issuer = process.env.MCP_OAUTH_ISSUER || baseUrl;
...
router.get(exports.RESOURCE_METADATA_PATH, (req, res) => {
    ...
    const authorizationServer = process.env.MCP_OAUTH_ISSUER || FALLBACK_AUTHORIZATION_SERVER;
    res.json({
        resource,
...
router.get(exports.AUTHORIZATION_SERVER_PATH, (req, res) => {
    ...
    res.json(buildAuthorizationServerMetadata(baseUrl));
});
// OpenID Provider Configuration (OIDC Discovery)
router.get(exports.OPENID_CONFIGURATION_PATH, (req, res) => {
    const baseUrl = resolveBaseUrl(req);
    res.json({
        ...buildAuthorizationServerMetadata(baseUrl),
        issuer: process.env.MCP_OAUTH_ISSUER || baseUrl,
