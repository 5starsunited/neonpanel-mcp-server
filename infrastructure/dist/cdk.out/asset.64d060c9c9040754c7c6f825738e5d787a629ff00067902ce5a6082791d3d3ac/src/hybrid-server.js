...
app.get('/openapi.json', (_req, res) => {
    try {
        const openapi = JSON.parse(node_fs_1.default.readFileSync(OPENAPI_PATH, 'utf8'));
        res.json(openapi);
    }
    catch (error) {
...
const OPENAPI_PATH = node_path_1.default.join(__dirname, '..', 'openapi.json');
...
async function requireBearer(req, res, next) {
    ...
    if (!token) {
        if (authHeader) {
            console.warn('[auth] Unsupported scheme', {
