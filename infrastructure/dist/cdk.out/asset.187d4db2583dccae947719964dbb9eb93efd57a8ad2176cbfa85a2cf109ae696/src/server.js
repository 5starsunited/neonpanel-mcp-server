...
async function keepaPost(path, apiKey, payload) {
    if (!KEEPA_MCP_BASE_URL)
        throw new Error('KEEPA_MCP_BASE_URL not configured');
    const url = `${KEEPA_MCP_BASE_URL.replace(/\/$/, '')}${path}`;
    ...
    try {
        ...
        switch (action) {
            ...
            case 'keepa.marketingResearch.priceHistory': {
                ...
                const apiKey = auth.replace(/^Bearer\s+/i, '');
                data = await keepaPost('/api/keepa/price-history', apiKey, p);
                break;
            }
            case 'keepa.marketingResearch.batchProductLookup': {
                ...
                const apiKey = auth.replace(/^Bearer\s+/i, '');
                data = await keepaPost('/api/keepa/batch-product-lookup', apiKey, p);
                break;
            }
            case 'keepa.marketingResearch.searchDeals': {
                ...
                const p = Schema.parse(args || {});
                const apiKey = auth.replace(/^Bearer\s+/i, '');
                data = await keepaPost('/api/keepa/search-deals', apiKey, p);
                break;
            }
            case 'keepa.marketingResearch.sellerLookup': {
                ...
                const p = Schema.parse(args || {});
                const apiKey = auth.replace(/^Bearer\s+/i, '');
                data = await keepaPost('/api/keepa/seller-lookup', apiKey, p);
                break;
            }
            case 'keepa.marketingResearch.bestSellers': {
                ...
                const p = Schema.parse(args || {});
                const apiKey = auth.replace(/^Bearer\s+/i, '');
                data = await keepaPost('/api/keepa/best-sellers', apiKey, p);
                break;
            }
            case 'keepa.marketingResearch.productFinder': {
                ...
                const p = Schema.parse(args || {});
                const apiKey = auth.replace(/^Bearer\s+/i, '');
                data = await keepaPost('/api/keepa/product-finder', apiKey, p);
                break;
            }
            case 'keepa.marketingResearch.categoryAnalysis': {
                ...
                const p = Schema.parse(args || {});
                const apiKey = auth.replace(/^Bearer\s+/i, '');
                data = await keepaPost('/api/keepa/category-analysis', apiKey, p);
                break;
            }
            case 'keepa.marketingResearch.salesVelocity': {
                ...
                const p = Schema.parse(args || {});
                const apiKey = auth.replace(/^Bearer\s+/i, '');
                data = await keepaPost('/api/keepa/sales-velocity', apiKey, p);
                break;
            }
            case 'keepa.marketingResearch.inventoryAnalysis': {
                ...
                const p = Schema.parse(args || {});
                const apiKey = auth.replace(/^Bearer\s+/i, '');
                data = await keepaPost('/api/keepa/inventory-analysis', apiKey, p);
                break;
            }
            case 'keepa.tokenStatus':
            case 'keepa.marketingResearch.tokenStatus': {
                const apiKey = auth.replace(/^Bearer\s+/i, '');
                data = await keepaPost('/api/keepa/token-status', apiKey, {});
                break;
            }
            default:
