...
function buildAuthorizationServerMetadata(req) {
    ...
    // DCR endpoint MUST be on MCP server since NeonPanel doesn't have one
    // Cross-host registration_endpoint is allowed by RFC 8414
    // ChatGPT and other clients will follow the endpoint regardless of host
...
router.post('/oauth2/register', (req, res) => {
    ...
    //   });
    // }
    const { redirect_uris, client_name, client_uri, logo_uri, scope, grant_types, response_types, token_endpoint_auth_method, jwks_uri, jwks } = req.body;
    ...
    const response = {
        ...
        registration_access_token: crypto_1.default.randomBytes(32).toString('hex')
    };
