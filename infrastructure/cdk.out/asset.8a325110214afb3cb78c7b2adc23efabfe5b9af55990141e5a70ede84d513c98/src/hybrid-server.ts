  ...
  try {
    ...
    switch (name) {
      ...
      case 'get_inventory_items': {
        ...
        const qs = buildQuery(rest);
        result = await neonpanelGet(`/api/v1/companies/${encodeURIComponent(companyUuid)}/inventory-items${qs}`, auth as string);
        break;
      }
      ...
      case 'get_item_cogs': {
        ...
        const qs = buildQuery(rest);
        result = await neonpanelGet(`/api/v1/companies/${encodeURIComponent(companyUuid)}/inventory-items/${encodeURIComponent(String(inventoryId))}/cogs${qs}`, auth as string);
        break;
      }
      ...
      case 'get_item_landed_cost': {
        const { companyUuid, inventoryId, ...rest } = toolArgs as any;
        const qs = buildQuery(rest);
        result = await neonpanelGet(`/api/v1/companies/${encodeURIComponent(companyUuid)}/inventory-items/${encodeURIComponent(String(inventoryId))}/landed-cost${qs}`, auth as string);
        break;
      }

      case 'get_revenue_and_cogs': {
        const qs = buildQuery(toolArgs as any);
        result = await neonpanelGet(`/api/v1/revenue-and-cogs${qs}`, auth as string);
        break;
      }
