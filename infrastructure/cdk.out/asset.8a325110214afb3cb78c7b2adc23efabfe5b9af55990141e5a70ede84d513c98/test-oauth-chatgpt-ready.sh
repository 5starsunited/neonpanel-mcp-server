...
if [[ ! "$REG_ENDPOINT" =~ ^$ISSUER ]]; then
  echo "   ℹ️  NOTE: registration_endpoint ($REG_ENDPOINT) is on different host than issuer"
  echo "   This is RFC 8414 compliant. NeonPanel doesn't have DCR, so MCP server provides it."
else
...
CAPABILITY_COUNT=$(echo "$CAPABILITIES" | jq '.metadata.total_capabilities // 0')
ACTION_COUNT=$(echo "$CAPABILITIES" | jq '.metadata.total_actions // 0')
echo "   Found $CAPABILITY_COUNT capabilities with $ACTION_COUNT total actions"
echo

if [[ "$CAPABILITY_COUNT" -eq 0 ]]; then
  echo "❌ FAIL: No capabilities found - ChatGPT needs at least one tool"
  exit 1
else
  echo "   ✅ Tools available for discovery"
  echo
  echo "   Sample capabilities:"
  echo "$CAPABILITIES" | jq '[.capabilities[0:3] | .[] | {capability: .capability_name, description: .description, action_count: (.actions | length)}]'
fi
echo
