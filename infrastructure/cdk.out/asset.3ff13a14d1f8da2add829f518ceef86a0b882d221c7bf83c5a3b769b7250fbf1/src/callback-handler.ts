 ...
 * 
 * This endpoint receives the authorization code from NeonPanel after user authorization.
 * 
 * Expected callback URL: https://mcp.neonpanel.com/callback
 * 
 * Query parameters:
 ...
 */
export function storeOAuthSession(sessionId: string, data: {
  codeVerifier: string;
  state: string;
  clientId: string;
  redirectUri: string;
 ...
 */
export function getOAuthSession(sessionId: string) {
...
async function exchangeCodeForToken(params: {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Token exchange failed: ${errorData.error || response.status} - ${errorData.error_description || response.statusText}`
    );
  }
...
router.get('/oauth/start', (req, res) => {
  ...
  authUrl.searchParams.set('scope', (scope as string) || 'read:inventory read:analytics');
