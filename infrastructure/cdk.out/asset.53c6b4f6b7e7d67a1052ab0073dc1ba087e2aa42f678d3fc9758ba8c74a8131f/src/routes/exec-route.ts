...
export interface ExecRouteOptions {
  ...
  attachAuthChallenge: (res: Response, req: Request, options?: { error?: string; scope?: string }) => void;
}
...
export function registerExecRoute(router: Router, options: ExecRouteOptions) {
  ...
  router.post('/exec', async (req, res) => {
    try {
      ...
      let validation: ValidatedAccessToken;

      try {
        validation = await validateAccessToken(rawToken);
      } catch (error) {
